...
"""Đọc log UART của board P4 qua cầu CH343 (115200 8N1) mà không cần `idf.py monitor`.
    ...
    python scripts/readlog.py --list                   # liệt kê cổng, đánh dấu cổng CH343 của board
    python scripts/readlog.py auto 60                  # tự tìm CH343, reset board rồi đọc 60 s ra stdout
    python scripts/readlog.py COM48 90 boot.log        # cổng cụ thể, ghi file (bytes thô, giữ mã màu)
    python scripts/readlog.py auto 120 live.log --no-reset   # không reset, bắt tiếp phiên đang chạy
...
Sau khi đọc xong, kiểm nhanh (CLAUDE.md §4 tiêu chuẩn "xong"):
