/**
 * button.c — quét 2 nút tích cực-thấp (pull-up ngoài 10 K trên board), chống dội 30 ms.
 * Kế thừa firmware-vimate-p4/main/input/button.c, thêm nút ĐO.
 */
#include "button.h"
#include "rapid4p.h"
...
static btn_t s_btns[] = {
    { .gpio = BOARD_BTN_BOOT_GPIO,    .tap_bit = R4P_EVT_BTN_PRESS,   .long_bit = R4P_EVT_BTN_LONG, .name = "BOOT", .last = 1 },
    { .gpio = BOARD_BTN_MEASURE_GPIO, .tap_bit = R4P_EVT_BTN_MEASURE, .long_bit = 0,                .name = "DO",   .last = 1 },
};
...
static void btn_poll(btn_t *b)
{
    ...
    if (lvl == 0 && b->last == 1) {
        ...
        if (!b->long_fired && (b->long_bit == 0 || held < LONG_MS)) {
        ...
        if (held >= LONG_MS) {
            ...
            ESP_LOGW(TAG_MAIN, "BTN %s giu %d s", b->name, LONG_MS / 1000);
        }
    }
}
...
esp_err_t button_init(void)
{
    ...
    ESP_LOGI(TAG_MAIN, "Button init BOOT=GPIO%d DO=GPIO%d", BOARD_BTN_BOOT_GPIO, BOARD_BTN_MEASURE_GPIO);
    return ESP_OK;
}
