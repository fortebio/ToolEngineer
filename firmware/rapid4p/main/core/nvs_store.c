...
/* Blob generic (Rapid4P thêm cho calib/led_pwm — app/calib_store.c). */
...
/* Trả ESP_ERR_NVS_NOT_FOUND khi chưa có; ESP_ERR_INVALID_SIZE khi cỡ khác `len`
 * (blob cũ khác layout → caller coi như chưa có, ghi lại mặc định). */
esp_err_t nvs_store_get_blob(const char *key, void *out, size_t len) {
    ...
    if (r != ESP_OK) return r;
    size_t got = 0;
    r = nvs_get_blob(h, key, NULL, &got);
    if (r == ESP_OK && got != len) r = ESP_ERR_INVALID_SIZE;
    if (r == ESP_OK) r = nvs_get_blob(h, key, out, &got);
    nvs_close(h);
    return r;
}
