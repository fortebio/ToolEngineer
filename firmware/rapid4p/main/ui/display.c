/**
 * display.c — phần cứng màn hình Rapid4P. Xem display.h.
 *
 * Nguồn: firmware-vimate-p4/main/ui/display.c (khối BOARD_LCD_USE_MIPI_DSI + DSI_ROTATE,
 * đã chạy thật trên board 11–15/09/2026). Giữ NGUYÊN trình tự và các ghi chú "vì sao"
 * — mỗi bước ở đây từng là một lỗi thật trên board.
 */
#include "display.h"
#include "rapid4p.h"
...
#error "display.c Rapid4P chi ho tro panel MIPI-DSI (BOARD_LCD_USE_MIPI_DSI 1)"
#endif
...
uint32_t display_flush_count(void) { return s_d.flush_count; }
...
esp_err_t display_init(void)
{
    ...
    /* LVGL task = task duy nhất rasterize + flush. Core 0 (UI), prio 6. */
    ...
    lvgl_port_lock(0);
#endif
    s_d.disp = lvgl_port_add_disp_dsi(&disp_cfg, &dsi_disp_cfg);
#if DSI_ROTATE
    if (s_d.disp) {
        esp_err_t rot_err = dsi_rot_attach(panel, s_d.disp);
    ...
    lv_display_add_event_cb(s_d.disp, flush_count_cb, LV_EVENT_FLUSH_FINISH, NULL);
