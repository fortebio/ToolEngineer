...
| Màn hình app | `main/ui/ui_reader.c`, `ui_strings.[ch]` | máy trạng thái 13 màn (theo ReaderPlus). Bố cục header · content · footer = thanh hành động (Quay lại/Huỷ LUÔN trái, hành động chính LUÔN phải teal, phá huỷ đỏ giữa + hộp thoại `confirm_show`). **Mọi kích thước qua token `UI_*` của `ui_theme.h`** — 2 thang: 4.3" (56/96/72/12, chữ 24/18/14, số khe 48) và 2.8" (`UI_SCALE_SMALL`: header 32 / footer **56** / nút 44 (softkey **50**, danh sách **56**, hộp thoại 52, ± ngưỡng 64×72) / gap 6, chữ 18/14, số khe 24 tự hạ qua `fit_font_from`; **mọi nút `lv_obj_set_ext_click_area(UI_EXT_CLICK)`** — khe hở giữa nút cũng nhận chạm, 2026-09-20 theo gợi ý người dùng "chỗ cảm ứng to hơn"); 2.8" header chỉ icon WiFi + số chờ gửi, IP/version xuống màn chính. **Footer 2.8" = SOFTKEY BAR 3 nút vật lý** (2026-09-20, theo vỏ máy Rapid/ReaderPlus: XANH · ĐỎ · TRẮNG dưới màn): `softkeys(g, r, w)` vẽ 3 ô thẳng hàng với 3 nút (vạch màu nút 4 px trên + nhãn 18 px `fit_font`; nhãn tiền tố `~` = phải GIỮ → ô 2 dòng "giữ"+nhãn, dùng `hold_lbl()`); chạm ô = nhấn nút (`SHORT_CLICKED` = tap, `LONG_PRESSED` = giữ, `lv_indev_set_long_press_time(BOARD_BTN_HOLD_MS)`). Hành động ở **`apply_key()` theo `s.state` — một nguồn sự thật**, nhãn ô ở `build_*` phải khớp. **Bảng softkey (tối ưu nông dân đeo găng, 2026-09-20)** — Chính: XANH Bắt đầu · ĐỎ **Đo lại** (mẫu+ống lần trước từ NVS `last_*`, ô rỗng nếu chưa có) · TRẮNG **giữ** Cài đặt; Đặt ống: XANH Quay lại · ĐỎ ĐO; Đang đo: ĐỎ **giữ** Dừng; Đo lỗi (`UI_MEASURE_ERROR`): XANH Thử lại · ĐỎ Quay lại; Kết quả: XANH Đo lại · ĐỎ **Xong** → Chính; Cài đặt 6 mục (Ngôn ngữ · WiFi · Cập nhật · Threshold · **Cân chỉnh** · Quay lại); Cân chỉnh: XANH Thoát (→ Cài đặt) · ĐỎ Cân chỉnh · TRẮNG **giữ** Xoá (+ hộp thoại); ngưỡng XANH +10 ĐỎ −10 TRẮNG Lưu (giữ = ±50 lặp / huỷ); WiFi: XANH Quay lại · ĐỎ Đổi mã (`ui_wifi_setup_toggle_card`); hộp thoại XANH Huỷ ĐỎ Xác nhận. **Chống nhầm**: khoá phím `KEY_LOCK_US` 350 ms sau mỗi `show()`; nhấn lúc màn ngủ chỉ đánh thức (`button.c wake_only`); 2 nút cùng lúc nhận 1 (`app_main` else-if, giữ > tap); giữ lặp `R4P_EVT_BTN_REPEAT` 400 ms chỉ có tác dụng ở ngưỡng/danh sách và khi màn chưa đổi (`s.hold_state`); ô nháy 150 ms (`sk_flash`) + `beep_key()` mỗi phím. Danh sách (`s.list_n > 0`): 3 cột × 2 hàng (`UI_BTN_LIST_W 94`), mục cuối "Quay lại", con trỏ `list_focus` = **tô nền teal + chữ tối** (viền mỏng không thấy từ xa — ảnh camera 2026-09-21) — ĐỎ ▼ TRẮNG ▲ XANH Chọn, chạm thẳng vẫn được. Màn chính 2.8": gợi ý 18 px gọi tên MÀU nút ("Nhấn nút XANH…"), IP·version ở màn Cập nhật; ô softkey rỗng không vạch. Đang đo: `remain_lbl` "còn ~N s" `F_HERO` (ước lượng từ thời gian trung bình mỗi bước, `remain_timer` 500 ms), "Đừng mở nắp", `display_note_user_activity()` mỗi `SLOT_START`; Kết quả: dòng tổng kết `F_HERO` "n/N DƯƠNG TÍNH" đỏ / "ÂM TÍNH N/N" xanh, ô khe nhuốm 30 % màu kết quả. Ngôn ngữ mặc định **VI**, menu chỉ `R4P_LANG_SELECTABLE` (2 khi chưa font CJK; NVS ≥ ZH về VI ở `calib_store_load`). Câu gợi ý dài dùng `mk_text()` (wrap, 100 %, canh giữa); không logo lớn/tên máy 48 px. Token màu/font = bản C của `system/brand/tokens.json` (`typography.scales`); `ui_theme_brand_bar()`, `mk_btn(icon, txt)` tự co font theo token, `mk_chip` |
...
| Nút | `main/input/button.c` | BOOT (tap/giữ 5 s), ĐO (P4), **3 nút XANH/ĐỎ/TRẮNG** (tap + giữ `BOARD_BTN_HOLD_MS` 1,5 s → `R4P_EVT_BTN_<X>[_LONG]`, giữ tiếp lặp 400 ms kèm `R4P_EVT_BTN_REPEAT` → `ui_reader_on_key_ex(key, hold, repeat)`; nhấn lúc màn ngủ = chỉ đánh thức, nuốt tap/giữ) — GPIO theo board (`BOARD_BTN_*`, −1 = không có) |
| **Bíp** | `main/audio/beep.[ch]` | S3 only (`CONFIG_RAPID4P_BEEP`, depends board S3): ES8311 0x18 trên I2C0 chung + I2S0 TX 24 kHz mono qua `esp_codec_dev` (^1.3 → 1.6.2), PA GPIO1 active-low điều khiển tay quanh mỗi tiếng (20 ms im → tone sin fade 5 ms → 20 ms im → PA off); task `beep` + queue, `beep_key/done/error` gọi từ mọi task; không thấy codec → WARN, no-op. P4: inline rỗng, không kéo component |
...
| Dev console | `main/core/dev_console.c`, `scripts/uicmd.py` (P4, UART0), **`scripts/keytest.py` (S3, USB-JTAG)**, **`scripts/lcdtool.py`** (web localhost) | REPL `r4p> `: `ui`/`btn do\|boot\|green\|red\|white [hold\|rep]`/`heap`/**`screen [raw]`** (chụp framebuffer LVGL `lv_snapshot_take` RGB565 → RLE (count u8, px u16 LE) → base64 76 ký tự/dòng giữa `SCR w h RGB565 RLE enc raw` và `SCR-END <crc32>`; ~25 KB/khung 320×240; chạy dưới `display_lock`, stack REPL 16 KB, buffer PSRAM). `btn` gọi `display_note_user_activity()` (đánh thức màn như nút thật). S3 2.8": `CONFIG_ESP_CONSOLE_USB_SERIAL_JTAG=y` (`sdkconfig.defaults.s3_28lcd`) → console + log cùng cổng COM USB-JTAG, `esp_console_new_repl_usb_serial_jtag`; P4 giữ UART0. `CONFIG_RAPID4P_DEV_CONSOLE` — TẮT ở bản phát hành |
...
| Build | `CMakeLists.txt`, `main/CMakeLists.txt`, `main/idf_component.yml`, `sdkconfig.defaults*`, `partitions/partitions.rapid4p.csv`, `scripts/build.bat` | ghim lvgl `~9.6.0`, esp_lvgl_port `~2.8.0`; P4: esp_hosted `2.12.*`, esp_wifi_remote `0.14.*` (`rules: target == esp32p4`); S3: `esp_lcd_ili9341 ^2.0.0`, `esp_codec_dev ^1.3.0` (`rules: target == esp32s3`; PRIV_REQUIRES nhánh else của `if(target STREQUAL "esp32p4")`) |
...
- **Audio (chỉ bíp)**: codec ES8311 0x18 trên I2C0 (chung touch + cảm biến), I2S0 MCLK4/BCLK5/WS7/DOUT8 (DIN6 mic không dùng),
  **PA GPIO1 ACTIVE-LOW**, 24 kHz 16-bit; loa nhỏ gắn vào ngõ PA của board (vỏ máy sẽ gắn loa — quyết định 2026-09-20).
  Cấu hình I2S y vimate (STEREO slot + SLOT_BOTH + bit_shift + MCLK ×256 — ES8311 đòi), `esp_codec_dev_open` channel=1.
...
- Console: **USB-Serial/JTAG là console chính** (2026-09-20, `CONFIG_ESP_CONSOLE_USB_SERIAL_JTAG=y` — bo không ra UART0 43/44):
  log + REPL dev console cùng COM 303A:1001 (`scripts/keytest.py COM20 "btn red"`); `readlog.py auto` KHÔNG tìm được (chỉ tìm CH343
  của P4) → ghi COM.
...
**Xem màn KHÔNG cần camera (từ 2026-09-21, phương án A `docs/plan/2026-09-21-lcd-debug-tool.md`)**:
...
| `screen` → `Guru Meditation … Double exception` trong `lv_malloc_core` | tràn stack task REPL (mặc định 4 KB) vì `lv_snapshot_take` vẽ lại cả màn trong task console → `repl_cfg.task_stack_size = 16384` (2026-09-21) |
...
> **Tool debug LCD localhost** (chưa chốt, đề xuất A: console `screen` dump framebuffer → `scripts/lcdtool.py` web):
> `docs/plan/2026-09-21-lcd-debug-tool.md` — làm trước khi rà UI/UX tiếp để bỏ vòng camera.
...
1. ~~Nạp bo ES3N28P 2.8"~~ (đã nạp 2026-09-19, log đạt §4 trừ phần mắt/chạm). Còn: **người cầm máy** xác nhận hướng xoay màn,
   ...
   **Nối 3 nút XANH/ĐỎ/TRẮNG của vỏ máy vào GPIO 47/48/41** (nhấn = GND) rồi kiểm softkey bằng nút cơ (mới kiểm bằng chạm và
