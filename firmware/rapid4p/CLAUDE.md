...
| Dev console | `main/core/dev_console.c`, `scripts/uicmd.py` (P4, UART0), **`scripts/keytest.py` (S3, USB-JTAG)** | REPL `r4p> `: `ui`/`btn do\|boot\|green\|red\|white [hold\|rep]`/`heap`. S3 2.8": `CONFIG_ESP_CONSOLE_USB_SERIAL_JTAG=y` (`sdkconfig.defaults.s3_28lcd`) → console + log cùng cổng COM USB-JTAG, `esp_console_new_repl_usb_serial_jtag`; P4 giữ UART0. `CONFIG_RAPID4P_DEV_CONSOLE` — TẮT ở bản phát hành |
