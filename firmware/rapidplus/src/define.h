...
struct parastructure {
  ...
  uint8_t topTemperatureSensorSq[4] = {0};  //hotlid sensor 1, 2, 3, ambient sensor. to be zero by default, need to calibrate it.
  ...
  double topOverheat[3] = {20,20,20};       //overheat value of top heater 
  float temperatureOffset[7] = {0};         //temperature offset of bottom sensor 1, 2, 3, hotlid sensor 1, 2, 3, ambient sensor, the usage is reading temperature + this value -> output temperature
  uint8_t hotlidPWM[3][2] = {{40, 80},{40, 80},{40, 80}};         //PWM low and high value for hotlid
...
#define info_displayf(...){DEBUG_COM.printf(__VA_ARGS__); SerialBT.printf(__VA_ARGS__);}
#define info_displayln(...) {DEBUG_COM.println(__VA_ARGS__); SerialBT.println(__VA_ARGS__);}
#define info_display(...) {DEBUG_COM.print(__VA_ARGS__); SerialBT.print(__VA_ARGS__);}
...
#define HOTLIDQUANTITY 4    //3 top temperature sensors plus 1 ambient temperature sensor located at PCB
