#ifndef DISPLAY_RESOURCES_H
#define DISPLAY_RESOURCES_H
#include "Arduino.h"

static const unsigned char logoFBT[] PROGMEM = {
    };

static const unsigned char shrimp[] PROGMEM = {
    };

static const unsigned char PROGMEM image_WIFI_Connect[] = {0x01, 0xf0, 0x00, 0x07, 0xfc, 0x00, 0x1e, 0x0f, 0x00, 0x39, 0xf3, 0x80, 0x77, 0xfd, 0xc0, 0xef, 0x1e, 0xe0, 0x5c, 0xe7, 0x40, 0x3b, 0xfb, 0x80, 0x17, 0x1d, 0x00, 0x0e, 0xee, 0x00, 0x05, 0xf4, 0x00, 0x03, 0xb8, 0x00, 0x01, 0x50, 0x00, 0x00, 0xe0, 0x00, 0x00, 0x40, 0x00, 0x00, 0x00, 0x00};

static const unsigned char PROGMEM image_WIFI_Disconnect[] = {0x21, 0xf0, 0x00, 0x16, 0x0c, 0x00, 0x08, 0x03, 0x00, 0x25, 0xf0, 0x80, 0x42, 0x0c, 0x40, 0x89, 0x02, 0x20, 0x10, 0xa1, 0x00, 0x23, 0x58, 0x80, 0x04, 0x24, 0x00, 0x08, 0x52, 0x00, 0x01, 0xa8, 0x00, 0x02, 0x04, 0x00, 0x00, 0x42, 0x00, 0x00, 0xa1, 0x00, 0x00, 0x40, 0x80, 0x00, 0x00, 0x00};

// display.drawRoundRect(15, 0, 302, 240, 10, 0xF206);
// display.drawRoundRect(30, 170, 272, 60, 10, 0xF206);
// display.drawCircle(55, 200, 20, 0xF206);

// display.setTextColor(0xF206);
// display.setTextColor(0xF206);
// display.setTextWrap(false);
// display.setFont(&FreeSans12pt7b);
// display.setCursor(92, 206);
// display.print("Setting Wifi");

// display.drawRoundRect(30, 94, 272, 60, 10, 0x4D6A);

// display.fillCircle(55, 200, 16, 0xF206);

// display.drawCircle(55, 124, 20, 0x4D6A);

// display.fillCircle(55, 124, 16, 0x4D6A);

// display.setTextColor(0x4D6A);
// display.setCursor(91, 132);
// display.print("Setting Language");

// display.drawBitmap(286, 9, image_download_bits, 19, 16, 0xFFFF);

// display.drawBitmap(286, 9, image_download_1_bits, 19, 16, 0xFFFF);

// display.display();

#endif
