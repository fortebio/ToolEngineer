...
bool checkJump(std::vector<double> &_array, double crossing, size_t index)
{
    ...
    // int drop = _array[index + 1] - _array[index];

    // 1. Tăng đột ngột
    if (jump < crossing)
        return false;

    // 2. Slope trung bình sau jump
...
void predict_outcome_break(Record &record, DiagnosticParameters &parameters)
{
    /*
    ...
    */
    ...
    // if (!check_breakData(record.raw_data, parameters.min_increase, parameters.detection_margin_time))
    // {
    ...
    // if (((record.outcome.plateau_point.x - record.outcome.transition_time.x) == 1) &&
    //     (record.outcome.increase > parameters.min_increase))
    // {
    //     strcpy(record.outcome.outcome, OutcomeBreak);
    // }
    return;
