...
#define TOPHEATER1PWMLOW   _ForteSetting.parameter.hotlidPWM[0][0]
#define TOPHEATER1PWMHIGH   _ForteSetting.parameter.hotlidPWM[0][1]
#define TOPHEATER2PWMLOW   _ForteSetting.parameter.hotlidPWM[1][0]
#define TOPHEATER2PWMHIGH   _ForteSetting.parameter.hotlidPWM[1][1]
#define TOPHEATER3PWMLOW   _ForteSetting.parameter.hotlidPWM[2][0]
#define TOPHEATER3PWMHIGH   _ForteSetting.parameter.hotlidPWM[2][1]
...
void PIDControl::loop()
{
    ...
    if(_topThermometer.getNewTemperatureFlag())
    {
        ...
        if ((_topThermometer.getTempSensorQuantity() != 4) && (! bheater1Simu))
        {
            info_displayf("Only got %d/4 top sensor resp\n", _topThermometer.getTempSensorQuantity());
            //if time out for 10 seconds, then stop all heating for safety concern
        ...
        const double targetTemp[4] = {HOTLID1_TEMP+OVERHEAT_THRESHOLD_TOP1, HOTLID23_TEMP+OVERHEAT_THRESHOLD_TOP2, HOTLID23_TEMP+OVERHEAT_THRESHOLD_TOP3, 50}; //last one is the ambient temperature
        for (uint8_t i = 0; i < 4; i++)
        {
            if (HotlidTemperature[i] == -127.0 + _ForteSetting.parameter.temperatureOffset[i+3])
            {
                ...
                info_displayf("\nTop heater: %.4g:%.4g:%.4g:%.4g\n", HotlidTemperature[0], HotlidTemperature[1], HotlidTemperature[2], HotlidTemperature[3]);
        ...
        if (btemperatureOut)
        {
            info_displayf("\nTimeRT\t%.2f\tTopHeater\tReading\t%.4g\t%.4g\t%.4g\t%.4g\n", millis()/1000.0, HotlidTemperature[0], HotlidTemperature[1], HotlidTemperature[2], HotlidTemperature[3]);
        }
    }
    else
    {
...
void PIDControl::sensorSeq()
{
    ...
    if ((topsensor[0] + topsensor[1] + topsensor[2]) == 0)     //simply check whther the sequence of top heater is set or not
    {
        //process bottom first
        uint8_t topHeaterPin[3] = {HOTLID1IO, HOTLID2IO, HOTLID3IO};
        ...
        if (_topThermometer.getTempSensorQuantity() != 4)
        {
            _topThermometer.begin();
        ...
        double topStartTemperature[4] = {100.0, 100.0, 100.0, 100.0};
        memcpy(topStartTemperature, _topThermometer.getTemperature(), 4*sizeof(double));
        info_displayf("top start temp: %.2f:%.2f:%.2f:%.2f\n", topStartTemperature[0], topStartTemperature[1], topStartTemperature[2], topStartTemperature[3]);
        uint8_t sumSeq = 0;     //this is used to find the sequence of ambient sensor
        for (uint8_t i = 0; i < 3; i++)     //control the 3 top heaters to find its sequence
        {
            //step 2. heat up
            ...
            while ( _ForteSetting.parameter.topTemperatureSensorSq[i] == 0xFF)
            {
                ...
                _topThermometer.clearNewTemperatureFlag();
                _displayCLD.TemperatureTopSeqDisplay();
                //check all 4 sensors, to get the right one with temperature higher than x degree
                ...
                for (uint8_t j = 0; j < 4; j++)
                {
                    ...
                    if(_topThermometer.getTemperature()[j] > topStartTemperature[j]+3)       //found it
                    {
                        //step 4. stop heat
        ...
        sumSeq = 1+2+3-sumSeq;
        if (sumSeq > 3 || sumSeq < 0)
        {
            info_displayf("The sequence result is wrong! %d\n", sumSeq);
        }
        _ForteSetting.parameter.topTemperatureSensorSq[3] = sumSeq;
    }
...
void PIDControl::Heat1Preheat80()
{
    ...
    else{       //if the temperature is between -1~+1
...
void PIDControl::MaintainHotlid1()
{
    if(!_topThermometer.getNewTemperatureFlag())       //if new temperature is not ready, then return directly.
    {
        return;
    ...
    if (bhotlid1Simu)
    {
        HotlidTemperature[0]  = HOTLID1_TEMP+0.5;
        stopAllHeating();
        return;
    }
    // if(HotlidTemperature[0]<HOTLID1_TEMP)              //heat high when tempeature is lower than target temperature
    // {
    //     analogWrite(HOTLID1IO, TOPHEATER1PWMHIGH);       //use high PWM value
    //     return;
    // }
    // else if (HotlidTemperature[0]>HOTLID1_TEMP+10)       //heat low when temperature is higher than 90
    // {
    //     analogWrite(HOTLID1IO, TOPHEATER1PWMLOW);       //use low PWM value
    // }

    //process top heater 2
    if (HotlidTemperature[0]>HOTLID1_TEMP+10)      //if heater2 is too high, in case the PWMLOW is not low enough
    {
        analogWrite(HOTLID1IO, PWM_OFF);
        if (btemperatureOut)
        {
            info_displayf("\nTimeMT1\t%.2f\tTopHeater1\tMaintain\tTemperature\t%.4g\tTarget\t%.2f\tPWM\t0\n", millis()/1000.0, HOTLID1_TEMP, HotlidTemperature[0]);
        }
    }
    else if (HotlidTemperature[0]>HOTLID1_TEMP+5)      //if heater2 is at high than 5 degree
    {
        analogWrite(HOTLID1IO, TOPHEATER1PWMLOW);
        if (btemperatureOut)
        {
            info_displayf("\nTimeMT1\t%.2f\tTopHeater1\tMaintain\tTemperature\t%.4g\tTarget\t%.2f\tPWM\t%d\n", millis()/1000.0, HotlidTemperature[0], HOTLID1_TEMP,TOPHEATER1PWMLOW);
        }
    }
    else if ((HotlidTemperature[0]<HOTLID1_TEMP-10))      //if heater2 is too low, in case the PWMHIGH is not high enough
    {
        analogWrite(HOTLID1IO, PWM_FULL);
        if (btemperatureOut)
        {
            info_displayf("\nTimeMT1\t%.2f\tTopHeater1\tMaintain\tTemperature\t%.4g\tTarget\t%.2f\tPWM\t255\n", millis()/1000.0, HotlidTemperature[0], HOTLID1_TEMP);
        }
    }
    else// if ((HotlidTemperature[0]<HOTLID1_TEMP))      //if heater2 is low
    {
        analogWrite(HOTLID1IO, TOPHEATER1PWMHIGH);
        if (btemperatureOut)
        {
            info_displayf("\nTimeMT1\t%.2f\tTopHeater1\tMaintain\tTemperature\t%.4g\tTarget\t%.2f\tPWM\t%d\n", millis()/1000.0, HotlidTemperature[0], HOTLID1_TEMP, TOPHEATER1PWMHIGH);
        }
    }
}
...
void PIDControl::heatNewLid23()
{
    ...
    if (HotlidTemperature[1]>HOTLID23_TEMP+10)      //if heater2 is too high
    {
        topHeater2Flag = 0;
        analogWrite(HOTLID2IO, PWM_OFF);
        info_displayf("TopHeater2 is too hot, tempearature: %.2f, target: %.2f, cooling down\n", HotlidTemperature[1], HOTLID23_TEMP);
    }
    else if ((HotlidTemperature[1]<HOTLID23_TEMP))      //if heater2 is too low
    {
        analogWrite(HOTLID2IO, PWM_HALF);
        info_displayf("\nTimePT2\t%.2f\tTopHeater2\tHeating\tTemperature\t%.2f\tTarget\t%.2f\tPWM\t127\n", millis()/1000.0, HotlidTemperature[1], HOTLID23_TEMP);
    }
    else if(HotlidTemperature[1]>HOTLID23_TEMP+5)        //if heater2 is higher than 5 degree, then using low PWM value
    {
        analogWrite(HOTLID2IO, TOPHEATER2PWMLOW);
        topHeater2Flag = 1;
        info_displayf("\nTimePT2\t%.2f\tTopHeater2\tMaintain\tTemperature\t%.2f\tTarget\t%.2f\tPWM\t%d\n", millis()/1000.0, HotlidTemperature[1], HOTLID23_TEMP, TOPHEATER2PWMLOW);
    }
    else        //if heater2 is at right range
    {
        analogWrite(HOTLID2IO, TOPHEATER2PWMHIGH);
        topHeater2Flag = 1;
        info_displayf("\nTimePT2\t%.2f\tTopHeater2\tMaintain\tTemperature\t%.2f\tTarget\t%.2f\tPWM\t%d\n", millis()/1000.0, HotlidTemperature[1], HOTLID23_TEMP, TOPHEATER2PWMHIGH);
    }
    
    if (HotlidTemperature[2]>HOTLID23_TEMP+10)      //if heater3 is too high
    {
        topHeater3Flag = 0;
        analogWrite(HOTLID3IO, PWM_OFF);
        info_displayf("TopHeater3 is too hot, tempearature: %.2f, target: %.2f, cooling down\n", HotlidTemperature[2], HOTLID23_TEMP);
    }
    else if ((HotlidTemperature[2]<HOTLID23_TEMP))      //if heater3 is too low
    {
        analogWrite(HOTLID3IO, PWM_HALF);
        // info_displayf("\nHeating hotlid3, tempearature: %.2f, target: %.2f, PID: 255\n", HotlidTemperature[2], HOTLID23_TEMP);
        info_displayf("\nTimePT3\t%.2f\tTopHeater3\tHeating\tTemperature\t%.2f\tTarget\t%.2f\tPWM\t127\n", millis()/1000.0, HotlidTemperature[2], HOTLID23_TEMP);
    }
    else if(HotlidTemperature[2]>HOTLID23_TEMP+5)        //if heater3 is higher than 5 degree, then using low PWM value
    {
        analogWrite(HOTLID3IO, TOPHEATER3PWMLOW);
        topHeater3Flag = 1;
        info_displayf("\nTimePT3\t%.2f\tTopHeater3\tMaintain\tTemperature\t%.2f\tTarget\t%.2f\tPWM\t%d\n", millis()/1000.0, HotlidTemperature[2], HOTLID23_TEMP, TOPHEATER3PWMLOW);
    }
    else        //if heater3 is at right range
    {
        analogWrite(HOTLID3IO, TOPHEATER3PWMHIGH);
        topHeater3Flag = 1;
        info_displayf("\nTimePT3\t%.2f\tTopHeater3\tMaintain\tTemperature\t%.2f\tTarget\t%.2f\tPWM\t%d\n", millis()/1000.0, HotlidTemperature[2], HOTLID23_TEMP, TOPHEATER3PWMHIGH);
    }
...
void PIDControl::HeatHotlid23()
{
    ...
    if (bhotlid23Simu)
    {
        temperatureSimulation(HotlidTemperature, 1, HOTLID23_TEMP);
        temperatureSimulation(HotlidTemperature, 2, HOTLID23_TEMP);
        stopAllHeating();
    }
    if (strcmp(_ForteSetting.parameter.PCB_version, "V1.3") < 0)        //if it's old PCB version
    {
...
{
    if ((HotlidTemperature[1]>HOTLID23_TEMP+10)||(HotlidTemperature[2]>HOTLID23_TEMP+10))       //Turn off when temperature is higher than 90
    {
        analogWrite(HOTLID23IO, PWM_OFF);
        if (btemperatureOut)
        {
...
{
    //process top heater 2
    if (HotlidTemperature[1]>HOTLID23_TEMP+10)      //if heater2 is too high, in case the PWMLOW is not low enough
    {
        analogWrite(HOTLID2IO, PWM_OFF);
        if (btemperatureOut)
        {
    ...
    else if (HotlidTemperature[1]>HOTLID23_TEMP+5)      //if heater2 is at high than 5 degree
    {
        analogWrite(HOTLID2IO, TOPHEATER2PWMLOW);
        if (btemperatureOut)
        {
    ...
    else if (HotlidTemperature[1]<HOTLID23_TEMP-10)      //if heater2 is too low, in case the PWMHIGH is not high enough
    {
        analogWrite(HOTLID2IO, PWM_Heater23);
        if (btemperatureOut)
        {
            info_displayf("\nTimeMT2\t%.2f\tTopHeater2\tMaintain\tTemperature\t%.2f\tTarget\t%.2f\tPWM\t%d\n", millis()/1000.0, HotlidTemperature[1], HOTLID23_TEMP, PWM_FULL);
        }
    }
    ...
    if (HotlidTemperature[2]>HOTLID23_TEMP+10)      //if heater3 is too high, in case the PWMLOW is not low enough
    {
        analogWrite(HOTLID3IO, PWM_OFF);
        if (btemperatureOut)
        {
            info_displayf("\nTimeMT3\t%.2f\tTopHeater3\tMaintain\tTemperature\t%.2f\tTarget\t%.2f\tPWM\t%d\n", millis()/1000.0, HotlidTemperature[2], HOTLID23_TEMP, PWM_OFF);
        }
    }
    else if (HotlidTemperature[2]>HOTLID23_TEMP+5)      //if heater3 is at high than 5 degree
    {
        analogWrite(HOTLID3IO, TOPHEATER3PWMLOW);
        if (btemperatureOut)
        {
    ...
    else if (HotlidTemperature[2]<HOTLID23_TEMP-10)      //if heater3 is too low, in case the PWMHIGH is not high enough
    {
        analogWrite(HOTLID3IO, PWM_Heater23);
        if (btemperatureOut)
        {
            info_displayf("\nTimeMT3\t%.2f\tTopHeater3\tMaintain\tTemperature\t%.2f\tTarget\t%.2f\tPWM\t%d\n", millis()/1000.0, HotlidTemperature[2], HOTLID23_TEMP, PWM_FULL);
        }
    }
...
void PIDControl::MaintainHotlid23()
{
    ...
    if (bhotlid23Simu)
    {
        HotlidTemperature[1] = HOTLID23_TEMP+0.5;
        HotlidTemperature[2] = HOTLID23_TEMP+0.5;
        stopAllHeating();
        return;
    }
