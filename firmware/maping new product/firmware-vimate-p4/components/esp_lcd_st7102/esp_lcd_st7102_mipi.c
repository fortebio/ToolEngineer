                                         ...
                                         esp_lcd_panel_handle_t *ret_panel)
{
    ...
    /* VIMATE: bo lenh doc ID (0x04). Doc DSI can panel tra loi BTA; HAL
     * mipi_dsi_hal_host_gen_read_short_packet() poll KHONG timeout nen panel
     * khong tra loi (ket trang thai cu, chua cap nguon, cap FPC long) la main
     * task quay vo han -> task_wdt, thiet bi khong bao gio boot xong (dinh that
     * 11/09/2026). ID chi de in log; bo di thi panel loi van boot duoc de giu
     * WS/OTA (display.c da thiet ke "LCD loi van boot"). */
