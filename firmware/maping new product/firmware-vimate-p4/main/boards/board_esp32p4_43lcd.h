/**
 * Board: VIMATE ESP32-P4 + 4.3" LCD ST7102 MIPI-DSI (480×800 portrait native).
 *
...
/* Cỡ mặt cảm xúc PNG dự phòng (px, cạnh vuông) trên màn ngang 800×480. 12/09: 400
 * (GIF Noto 160 px × 2,5). Từ 15/09 chỉ còn dùng cho PNG 128 px khi GIF mặt lỗi. */
#define BOARD_EMOJI_TARGET_PX        400
/* 15/09/2026: "mặt robot" toàn màn (README-P4 §6.5b) — 32 clip GIF 400×240 nền đen
 * trong emo_spiffs (partitions.p4-43lcd.csv, 5,375 MB), nhân đôi pixel = 800×480 đúng
 * màn logical, vẽ thẳng RGB565 không alpha, chỉ vẽ lại hộp bẩn mỗi khung. Sàn 40 ms
 * (S3 kẹp 150) = mức decode kịp (đo 9–13 ms/khung); tool build_face_gifs.py gộp khung
 * nguồn ngắn hơn sàn này nên đổi sàn phải dựng lại bộ. Idle: nhìn quanh/chán mỗi
 * 15–30 s, buồn ngủ sau 45 s rảnh — đều dưới 60 s đồng hồ màn chờ. */
#define BOARD_FACE_GIF_FULLSCREEN    1
...
/* ui_image_hide(): DSI/PPA vẽ cả màn ~16 ms → 40 ms là đủ 2 khung để display task rời
 * src ảnh cũ trước khi free. Mặc định 600 (S3 SPI) làm MỌI lần chạm điều hướng trên P4
 * ngủ 0,6 s (log 13/09: "Mic OFF" → "→ abort" luôn cách 600 ms). */
#define BOARD_UI_IMAGE_UNHOOK_MS     40
...
/* GPIO0 = BTN3. Hiện KHÔNG có code nào đọc BOARD_BTN_STOP_GPIO — để lại làm tư
 * liệu, bật lên khi viết driver nút thứ hai. */
...
/* ========================== Board metadata ========================== */
#define BOARD_NAME                   "vimate_p4_st7102_lcd43"
