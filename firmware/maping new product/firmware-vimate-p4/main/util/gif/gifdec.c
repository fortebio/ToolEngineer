...
static gd_GIF * gif_open(gd_GIF * gif_base)
{
    ...
    size_t gif_alloc_size = sizeof(gd_GIF) + 5 * width * height + LZW_CACHE_SIZE;
#else
    if(0 == (INT_MAX - sizeof(gd_GIF)) / width / height / 5){
        ESP_LOGW(TAG, "Image dimensions are too large");
        goto fail;
    } 
    size_t gif_alloc_size = sizeof(gd_GIF) + 5 * width * height;
#endif
    gif = heap_caps_malloc(gif_alloc_size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if(!gif) {
        gif = heap_caps_malloc(gif_alloc_size, MALLOC_CAP_8BIT);
    }
    if(!gif) {
        ESP_LOGE(TAG, "heap OOM for %ux%u GIF (need %u bytes)",
                 width, height, (unsigned)gif_alloc_size);
        goto fail;
    }
...
read_image_data(gd_GIF * gif, int interlace)
{
	...
	if(frm_off + str_len > frm_size){
        ...
        /* VIMATE 15/09/2026: chuỗi LZW được ghi ngược (p giảm 1 mỗi bước vì
         * entry.length giảm 1 khi đi về prefix) → tính x,y một lần rồi lùi từng
         * pixel, thay vì % và / cho MỖI pixel (RISC-V P4: ~30 chu kỳ/phép; khung
         * 46 K px tốn ~4 ms chỉ cho chia). Interlace: chỉ đổi hàng khi y đổi. */
        p = frm_off + str_len - 1;
        x = p % gif->fw;
        y = p / gif->fw;
        {
            int yy = interlace ? interlaced_line_index((int) gif->fh, y) : y;
            uint8_t * row = &gif->frame[(gif->fy + yy) * gif->width + gif->fx];
            for(i = 0; i < str_len; i++) {
                row[x] = entry.suffix;
                ...
                if(x == 0) {
                    x = gif->fw - 1;
                    y--;
                    yy = interlace ? interlaced_line_index((int) gif->fh, y) : y;
                    row = &gif->frame[(gif->fy + yy) * gif->width + gif->fx];
                } else {
                    x--;
                }
            }
...
dispose(gd_GIF * gif)
{
    ...
    switch(gif->gce.disposal) {
        case 2: /* Restore to background color. */
            if(gif->render_hook) {
                gif->render_hook(gif, gif->render_user, gif->fx, gif->fy, gif->fw, gif->fh,
                                 gif->bgindex);
                break;
            }
            bgcolor = &gif->palette->colors[gif->bgindex * 3];
        ...
        default:
             ...
             * VIMATE 15/09/2026: bỏ — lvgl_gif.c LUÔN gọi gd_render_frame() ngay sau
             * gd_get_frame() nên canvas đã chứa khung này; vẽ lại ở đây là tốn một lần
             * tra bảng màu + ghi PSRAM cho cả rect (đo trên P4: ~1/3 thời gian khung
             * 400×240). Gốc xiaozhi giữ để caller không render vẫn đúng. */
            break;
    }
}
...
{
    if(gif->render_hook) {
        gif->render_hook(gif, gif->render_user, gif->fx, gif->fy, gif->fw, gif->fh, -1);
        return;
    }
