    ...
    if (base == WIFI_EVENT) {
        switch (id) {
            ...
            case WIFI_EVENT_STA_DISCONNECTED:
                ...
                xEventGroupClearBits(g_vimate_events, VIMATE_EVT_WIFI_UP);
                xEventGroupSetBits(g_vimate_events, VIMATE_EVT_WIFI_DOWN);
        ...
        xEventGroupSetBits(g_vimate_events, VIMATE_EVT_WIFI_UP);
        xEventGroupClearBits(g_vimate_events, VIMATE_EVT_WIFI_DOWN);
    }
}
 ...
 *
 * UX (theo xiaozhi):
 *   1. Phone join open network "<VIMATE_SETUP_PREFIX>-XXXX" (brand hiện tại: GENU-Setup)
...
"<title>" VIMATE_BRAND_NAME " - Cai dat WiFi</title><style>"
...
"body:JSON.stringify({ssid:s,password:$('pwbox').hidden?'':p})})"
...
esp_err_t wifi_mgr_start_provisioning(void) {
    ...
    snprintf(ap_ssid, sizeof(ap_ssid), VIMATE_SETUP_PREFIX "-%s", mac + 12);
    ...
    if (xTaskCreate(prov_prewarm_task, "wifi_prewarm", 3072, NULL,
                    VIMATE_TASK_PRIO_NETWORK, NULL) != pdPASS) {
